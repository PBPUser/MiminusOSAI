...
/// <summary>Builds the player's music: short looping chiptunes generated from a
/// seed, so the playlist has real audio without shipping any.</summary>
public static class Chiptune
{
    ...
    public static (short[] pcm, int rate) Build(int seed, double seconds)
    {
        ...
        for (int i = 0; i < beats; i++)
        {
            ...
            if (i % 2 == 0)
                w.Square((float)Wave.Note(chord - 24), 0.20f, (float)t, (float)(beat * 1.6), 0.5f, 0.004f, 1.2f);

            // Arpeggiated lead.
            int[] offsets = { 0, 4, 7, 12 };
            int note = chord + offsets[rng.Next(offsets.Length)];
            w.Triangle((float)Wave.Note(note), 0.16f, (float)t, (float)(beat * 0.9), 0.004f, 1.6f);

            // Counter-melody an octave up, sparsely.
            if (rng.NextDouble() < 0.35)
                w.Square((float)Wave.Note(note + 12), 0.07f, (float)(t + beat * 0.5), (float)(beat * 0.45),
                         0.25f, 0.003f, 2f);
